...
func (tgEnv *TelegramBotEnvironment) Start(ctx context.Context) error {
	tgEnv.logger.Info("Telegram bot started")
	go func() {
		<-ctx.Done()
		tgEnv.Bot.Stop()
	}()
	tgEnv.Bot.Start() // the call is blocking
	tgEnv.logger.Info("Telegram bot finished")
	return nil
}
...
func (tgEnv *TelegramBotEnvironment) SubscribeToAllAlerts() error {
	for alertType, alertName := range entities.GetAllAlertTypesAndNames() {
		...
		tgEnv.logger.Info("Telegram bot subscribed to alert", slog.String("alertName", string(alertName)))
	}
	return nil
}
...
func (tgEnv *TelegramBotEnvironment) UnsubscribeFromAlert(alertType entities.AlertType) error {
	...
	tgEnv.logger.Info("Telegram bot unsubscribed from alert", slog.String("alertName", string(alertName)))
	return nil
}
...
func executeTemplate(templateName string, data any, extension ExpectedExtension) (string, error) {
	switch extension {
	case HTML:
		tmpl, err := htmltmpl.ParseFS(templateFiles, templateName+string(extension))
		if err != nil {
			return "", err
		}
		buffer := &bytes.Buffer{}
	...
	case Markdown:
		tmpl, err := textmpl.ParseFS(templateFiles, templateName+string(extension))
		if err != nil {
			return "", err
		}
		buffer := &bytes.Buffer{}
