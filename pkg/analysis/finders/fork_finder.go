...
func (f *ForkFinder) tryLinearStateHashSearch(
...
) (uint64, entities.StateHash, error) {
	if f.linearSearchParams == nil {
		return 0, entities.StateHash{}, errors.Wrap(errNotFound, "no linear search params provided")
	}
	for i := uint64(0); i < f.linearSearchParams.searchDepth && stop-start > i; i++ {
		...
		if err != nil {
			...
			return 0, entities.StateHash{}, errors.Wrapf(err,
		...
		if err != nil {
			return 0, entities.StateHash{}, errors.Wrapf(err, "no statehash at height %d", h)
		}
		return h, sh, nil
	}
	return 0, entities.StateHash{}, errors.Wrapf(errNotFound, "linear search failed for nodes '%s' and '%s'", nodeA, nodeB)
}

func (f *ForkFinder) FindLastCommonStateHash(nodeA, nodeB string) (uint64, entities.StateHash, error) {
	startA, err := f.storage.EarliestHeight(nodeA)
	if err != nil {
		return 0, entities.StateHash{}, errors.Wrapf(err, "no earliest statement for node '%s'", nodeA)
	}
	startB, err := f.storage.EarliestHeight(nodeB)
	if err != nil {
		return 0, entities.StateHash{}, errors.Wrapf(err, "no earliest statement for node '%s'", nodeB)
	}
	start := max(startA, startB)

	stopA, err := f.storage.LatestHeight(nodeA)
	if err != nil {
		return 0, entities.StateHash{}, errors.Wrapf(err, "no latest statement for node '%s'", nodeA)
	}
	stopB, err := f.storage.LatestHeight(nodeB)
	if err != nil {
		return 0, entities.StateHash{}, errors.Wrapf(err, "no latest statement for node '%s'", nodeB)
	}
	stop := min(stopA, stopB)
	...
	for start <= stop {
		...
		if stgErr != nil {
			...
			return 0, entities.StateHash{}, stgErr
		}
		if different {
			stop = middle - 1
			r = stop
	...
	initialStop := min(stopA, stopB)
	if r < initialStart {
		return 0, entities.StateHash{}, errors.Wrapf(ErrNoCommonBlocks,
			"no common blocks in range [%d, %d]", initialStart, initialStop,
		)
	}
	sh, err = f.storage.StateHashAtHeight(nodeA, r)
	if err != nil {
		return 0, entities.StateHash{}, errors.Wrapf(err, "no statehash ID at height %d", r)
	}
	return r, sh, nil
}
